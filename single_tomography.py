...
Monte Carlo Simulation of the Setup Used for 1-Qubit Tomography
"""
import numpy as np
...
# [H, V, P, M, R, L]
max_runs = 10000
reference_density = np.zeros((2, 2), dtype=np.complex128)
error_samples = []
...
for run in range(max_runs):
    ...
    rho = density_matrix.reshape(2, 2, 2, 2)
    density_reduced = np.trace(rho, axis1=1, axis2=3)
    if run == 0:
        reference_density = density_reduced
        print(' T-matrix:\n', T, '\n')
    else:
        error_samples.append(density_reduced)
